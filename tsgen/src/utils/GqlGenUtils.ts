...
import { extractBusinessFieldNodeList } from "./BaseUtils"
    ...
    if (fdn.type.kind === "NonNullType"
        ...
        if (businessObjectTypeNode) {
            if ((!simpleMode)) {
                ...
                const businessFieldList = extractBusinessFieldNodeList(businessObjectTypeNode)
...
export const getFragment = (
    ...
    const businessFieldList = extractBusinessFieldNodeList(node)

    if (businessFieldList.length > 0) {
...
export const getUpdateOrCreateMutation = (inputNode: InputObjectTypeDefinitionNode): DocumentNode | undefined => {
